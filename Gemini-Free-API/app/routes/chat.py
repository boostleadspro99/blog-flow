...
async def chat_completions(
    ...
    if not cookie_row:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No Gemini cookies configured.")
    ...
    pooled = await gemini_pool.get_or_create(str(current_user.id), secure_1psid, secure_1psidts)
    try:
    ...
    raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail="Chat completions with per-user auth is coming soon.")
